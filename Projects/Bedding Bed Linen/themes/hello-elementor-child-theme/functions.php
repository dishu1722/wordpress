 ...
 * Use [daily_hire_calculator] in Elementor.
 */
...
function add_calculator_items_to_cart_callback() {
    ...
    $quantity   = max(1, intval($item['quantity'])); // minimum 1
...
// Make hire products purchasable even if regular price is empty
add_filter('woocommerce_is_purchasable', function($purchasable, $product){
    // Only for products with ACF custom prices
    $pickup_price     = get_field('pickup_price', $product->get_id());
    $daily_hire_price = get_field('daily_hire_price', $product->get_id());

    if ( empty($product->get_price()) && ($pickup_price || $daily_hire_price) ) {
        return true;
    }
