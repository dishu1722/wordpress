jQuery(document).ready(function($) {
    ...
    $('#add-to-cart-button').on('click', function(e) {
        e.preventDefault();
        var cartItems = [];
        var firstActiveDays = syncedDays || 1;
        ...
        if (cartItems.length > 0) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        alert('Error: ' + (response.data.message || 'Could not add items to cart.'));
            ...
            alert('Please select at least one item and set a quantity.');
